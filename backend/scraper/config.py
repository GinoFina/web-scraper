...
def _load_tournament_names() -> dict[int, str]:
    from core.paths import USER_DIR
    path = USER_DIR / "tournament_names.json"
...
TOURNAMENT_NAMES: dict[int, str] = _load_tournament_names()

def save_custom_tournament_name(tournament_id: int, custom_name: str):
    from core.paths import USER_DIR
    path = USER_DIR / "tournament_names.json"
    
    names = _load_tournament_names()
    names[tournament_id] = custom_name
    
    with open(path, "w", encoding="utf-8") as f:
        json.dump({str(k): v for k, v in names.items()}, f, indent=2)
        
    global TOURNAMENT_NAMES
    TOURNAMENT_NAMES[tournament_id] = custom_name
